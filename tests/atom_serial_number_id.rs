...
fn get_atoms_by_model(filename: &str) -> Vec<Vec<Atom>> {
    get_structure_with_errors(filename).0.models().map(|model| model.atoms().cloned().collect()).collect()
}

fn get_atoms(filename: &str) -> Vec<Atom> {
    get_structure_with_errors(filename).0.atoms().cloned().collect()
}
...
fn atom_serial_number_only_unique_within_model_mmcif() {
    ...
    assert!(
        get_atoms_by_model(filename_shared_ids)
            .iter()
            .all(|atoms|
                atoms.iter().map(|atom| atom.serial_number()).all_unique()
            )
    );
    assert!(! get_atoms(filename_shared_ids).iter().map(|atom| atom.serial_number()).all_unique());

    let filename_unique_ids = "example-pdbs/3pdz.cif";
    assert!(
        get_atoms_by_model(filename_unique_ids)
            .iter()
            .all(|atoms|
                atoms.iter().map(|atom| atom.serial_number()).all_unique()
            )
    );
    assert!(! get_atoms(filename_unique_ids).iter().map(|atom| atom.serial_number()).all_unique());
}
...
fn atom_serial_number_only_unique_within_model_pdb() {
    ...
    assert!(
        get_atoms_by_model(filename)
            .iter()
            .all(|atoms|
                atoms.iter().map(|atom| atom.serial_number()).all_unique()
            )
    );
    assert!(! get_atoms(filename).iter().map(|atom| atom.serial_number()).all_unique());
}

#[test]
fn atom_id_unique_globally_pdb() {
    let filename = "example-pdbs/pTLS-6484.pdb";
    assert!(get_atoms(filename).iter().map(|atom| atom.id()).all_unique());
}
...
fn warning_if_atom_ids_not_unique_mmcif() {
    ...
    assert!(! structure.atoms().map(|atom| atom.id()).all_unique());
    assert!(errors.iter().any(|err| err.short_description().contains("Duplicated atom IDs")));
}